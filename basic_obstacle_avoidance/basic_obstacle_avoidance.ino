...
const int SIDE_TRIGGER_PIN = 6; //D10 pin
const int SIDE_ECHO_PIN = 7; //D11 pin
...
const int FRONT_TRIGGER_PIN = 5; //D6 pin
const int FRONT_ECHO_PIN = 4; //D7 pin
...
void check(){
   ...
   if ((current_distance > 30 || current_distance == 0) && (side_distance > 30 || side_distance == 0)) {
    car.setSpeed(40);
    car.setAngle(0);
  }
   else if ((current_distance > 0 && current_distance < 30) || (side_distance > 0 && side_distance < 30)) {
     car.setSpeed(0);
   }
  }
